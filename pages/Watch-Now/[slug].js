...
const Watch = ({ anime, animeD, AllAnimeData}) => {
  ...
  // Filter servers where the episode exists
const filterServers = (episodeNumber) => {
  const serversWithEpisode = Object.keys(MyAnimeData.episodes).filter((server) =>
    MyAnimeData.episodes[server].some(
      (ep) => ep.episodeNumber === episodeNumber
    )
  );
  setFilteredServers(serversWithEpisode);
};

// Initialize Default Server and Episode
useEffect(() => {
  setHydrated(true); 
  if (MyAnimeData.episodes) {
    // Prefer 'Multi Server', else pick the first available server
    const defaultServer = MyAnimeData.episodes["Multi"]
      ? "Multi"
      : Object.keys(MyAnimeData.episodes)[0];

    setCurrentServer(defaultServer);

    // Set the first episode of the selected server
    const firstEpisode = MyAnimeData.episodes[defaultServer]?.[0];
    if (firstEpisode) {
      setCurrentEpisode(firstEpisode);

      // Filter servers for the first episode
      filterServers(firstEpisode.episodeNumber);
    }
  }
}, [MyAnimeData.episodes]);
...
const handleEpisodeClick = (episodeNumber) => {
  // Find the first server that hosts the episode
  const availableServer = Object.keys(MyAnimeData.episodes).find((server) =>
    MyAnimeData.episodes[server].some(
      (ep) => ep.episodeNumber === episodeNumber
    )
  );
  ...
  if (availableServer) {
    // Set the episode from the available server
    const episodeToPlay = MyAnimeData.episodes[availableServer].find(
      (ep) => ep.episodeNumber === episodeNumber
    );

    setCurrentServer(availableServer);
    setCurrentEpisode(episodeToPlay);

    // Filter servers for this episode
    filterServers(episodeNumber);
  }
};
                      ...
                      <div className="episode-server-section">
                        ...
                        <div className="selact-servers">
                          <div className="servers-div">
                              ...
                              {filteredServers.map((serverType) => (
        <span
          key={serverType}
          ...
          onClick={() => {
            setCurrentServer(serverType); // User manually selects a server
            const firstEpisodeFromServer = MyAnimeData.episodes[serverType]?.[0];
            if (firstEpisodeFromServer) {
              setCurrentEpisode(firstEpisodeFromServer); // Play the first episode of selected server
            }
          }}
        >
          {serverType}
              ...
              <div className="list-container">
                <div className="episodes-list">
                  ...
                  >
                    ...
                    {[...new Map(
        Object.values(MyAnimeData.episodes)
          .flat()
          .map((episode) => [episode.episodeNumber, episode]) // Use episodeNumber as key
      ).values()].map((uniqueEpisode, index) => {
                    ...
                    return (
                      ...
                      <div
                        ...
                        key={index}
                        onClick={() => {
                          const availableServers = Object.keys(MyAnimeData.episodes).filter(
                            (server) =>
                              MyAnimeData.episodes[server]?.some(
                                (ep) => ep.episodeNumber === uniqueEpisode.episodeNumber
                              )
                          );
              
                          // If the current server doesn't have this episode and user hasn't manually selected a server, switch
                          if (
                            !MyAnimeData.episodes[currentServer]?.some(
                              (ep) => ep.episodeNumber === uniqueEpisode.episodeNumber
                            )
                          ) {
                            const preferredServer = availableServers.includes("Multi")
                              ? "Multi"
                              : availableServers[0]; // Default to Multi or first available server
                            setCurrentServer(preferredServer);
                          }
              
                          setCurrentEpisode(uniqueEpisode); // Set the clicked episode
                        }}
                      >
                        <div className="short-anime-ep">
                          <div
                            className={`episode-num ${isEven
                              ...
                              currentEpisode?.episodeNumber === uniqueEpisode.episodeNumber
                                ? "active-episode"
                                : ""
                            }`}
                          >
                            {uniqueEpisode.episodeNumber}
                          </div>
                          <div
                            ...
                            >
                            <span>{uniqueEpisode.EpisodeTitle}</span>
                          </div>
                        </div>
                      </div>
                    )})}
